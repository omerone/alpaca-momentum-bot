"""Sizing + trailing-stop experiments on the live strategy.
...
# (label, trail cap %, risk % of equity, max position $, max positions)
# None = keep the live config value.
VARIANTS = [
    ("בלי תקרה (נוכחי)",  None, None, None, None),
    ("תקרה 1.6%",          1.6, None, None, None),
    ("תקרה 1.4%",          1.4, None, None, None),
    ("תקרה 1.2%",          1.2, None, None, None),
    ("תקרה 1.1%",          1.1, None, None, None),
    ("תקרה 1.0%",          1.0, None, None, None),
]
...
def replay_day(day_data, atrs, spy_df, cap_pct, risk, max_value, max_pos) -> list:
    ...
    for t in all_times:
        ...
        if not (WINDOW[0] <= t.time() < WINDOW[1]) or len(open_pos) >= max_pos:
            continue
...
def run_period(symbols, start, end) -> dict:
    ...
    out = {}
    for label, cap, risk_pct, max_value, max_pos in VARIANTS:
        risk = EQUITY * (config.risk_pct_per_trade if risk_pct is None else risk_pct) / 100
        max_value = MAX_VALUE if max_value is None else max_value
        max_pos = config.max_positions if max_pos is None else max_pos
        trades = []
        for day in days:
            ...
            trades += replay_day(day_data, atrs, spy_day if spy_day is not None and len(spy_day) else None,
                                 cap, risk, max_value, max_pos)
