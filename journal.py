...
class TradeJournal:
    ...
    def breakdown(self, since: str | None = None, tz=None) -> dict:
        ...
        """
        rows = self.recent(limit=100_000, since=since)
        rows.reverse()                     # oldest first, for the cumulative curve
    ...
    def stats(self, since: str | None = None) -> dict:
        ...
        try:
            with self._conn() as c:
                rows = c.execute(
                    f"SELECT pnl_usd, pnl_pct, held_seconds FROM trades{where}", args
                ).fetchall()
        except Exception as e:
